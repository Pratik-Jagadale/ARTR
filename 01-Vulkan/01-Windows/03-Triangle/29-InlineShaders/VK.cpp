...
// GL/SPIRV RELATED HEADER FILES
#include <SPIRV/GlslangToSpv.h>
#include <Public/ShaderLang.h> // RECENTLY REQURED
// #include <Public/ShaderLang.h> // RECENTLY REQURED
...
// GLSLLANG/SPIRV RELATED LIBS
#pragma comment(lib, "glslang.lib") // RECENTLY REQURED
...
const char *enabledInstanceExtensionNames_Array[3]; // VK_KHR_SURFACE_EXTENSION_NAME & VK_KHR_WIN32_SURFACE_EXTENSION_NAME & VK_EXT_DEBUG_REPORT_EXTENSION_NAME
...
VkResult createShader(void)
{
	...
	// CODE
	// INITIALIZE GLSL LANG PROCEDURES
	...
	if (bDone == FALSE)
	{
		fprintf(gpFile, "%s => glsl2spv() IS FAILED FOR VERTEX SHADER.\n", __func__);
		vkResult = VK_ERROR_INITIALIZATION_FAILED;
		return (vkResult);
	}
	else
	{
		fprintf(gpFile, "%s => glsl2spv() IS SUCCEEDED FOR VERTEX SHADER.\n", __func__);
	}
	...
	if (bDone == FALSE)
	{
		fprintf(gpFile, "%s => glsl2spv() IS FAILED FOR FRAGMENT SHADER.\n", __func__);
		vkResult = VK_ERROR_INITIALIZATION_FAILED;
		return (vkResult);
	}
	else
	{
		fprintf(gpFile, "%s => glsl2spv() IS SUCCEEDED FOR FRAGMENT SHADER.\n", __func__);
	}
...
BOOL glsl2spv(const VkShaderStageFlagBits shader_type, const char *pShader, vector<unsigned int> &spirv)
{
	...
	// CREATE T SHADER TYPE SHADER FROM ABOVE EShLanguage
	...
	// INTIALIZE BUILD IN SHADER RESOURCES
	TBuiltInResource resources;

	initBuiltInResources(resources);

	// SET OUR SHADER STRING IN ABOVE ARRAY AS T SHADER STRING's
	shader.setStrings(shaderStrings, 1);

	// SPECIFY RULES OF SHADER  COMPILATION AS SPIRV RULES & VULKAN RULES
	EShMessages ruleMessages = (EShMessages)(EShMsgSpvRules | EShMsgVulkanRules);

	// PARSE THE T SHADER TO COMPILE
	if (!shader.parse(&resources,
					  100,	 // VERSON DEFAULT
					  false, // FORWARD COMPATIBLE?
					  ruleMessages))
	{
		fprintf(gpFile, "%s => Parse() IS FAILED WITH LOG %s.\n", __func__, shader.getInfoLog());

		fprintf(gpFile, "%s => Parse() IS FAILED WITH DEBUG LOG %s.\n", __func__, shader.getInfoDebugLog());
		return FALSE;
	}
	else
	{
		fprintf(gpFile, "%s => PARSE() IS SUCCEEDED.\n", __func__);
	}
	...
	{
		fprintf(gpFile, "%s => link() IS FAILED WITH LOG %s.\n", __func__, program.getInfoLog());

		fprintf(gpFile, "%s => link() IS FAILED WITH DEBUG LOG %s.\n", __func__, program.getInfoDebugLog());
		return FALSE;
	}
	else
	{
		fprintf(gpFile, "%s => LINk() IS SUCCEEDED.\n", __func__);
	}

	// AND NOW SET WHOLE COOKED PROGRAM & SPRIVE TO GLSL LANG LIB
